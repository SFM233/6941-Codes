...
class MyRobot(wpilib.IterativeRobot):
    ...
    def autonomousPeriodic(self):
        ...
        if self.gameData[0:1] == "L":
            ...
            timer.reset()
            while timer.get() < Left:
                self.myRobot.tankDrive(0.7,-0.7) # turn left, although looks like turn right
            while timer.get() < 1.1:
                self.myRobot.tankDrive(0,0)
            timer.reset()
            while timer.get() < 0.5:
            ...
            while timer.get() < 0.8:
                self.myRobot.tankDrive(0,0)
            timer.reset()
            while timer.get() < Right:
                self.myRobot.tankDrive(-0.7,0.7) # turn right
            while timer.get() < 1:
                self.myRobot.tankDrive(0,0) # push pneumatics to switch position
            timer.reset()
            while timer.get() < 0.4:
                self.myRobot.tankDrive(0.7,0.7) # drop cube
            while timer.get() < 5:
                self.ihigh_motor.set(-0.8) # outtake
            ...
            timer.reset()
            while timer.get() < Right:
                self.myRobot.tankDrive(-0.7,0.7) # turn right, although looks like turn left
            while timer.get() < 1.1:
                self.myRobot.tankDrive(0,0)
            timer.reset()
            while timer.get() < 0.5:
            ...
            timer.reset()
            while timer.get() < Left:
                self.myRobot.tankDrive(-0.7,0.7) # turn left
            while timer.get() < 1:
                self.myRobot.tankDrive(0,0) # push pneumatics to switch position
            timer.reset()
            while timer.get() < 0.4:
