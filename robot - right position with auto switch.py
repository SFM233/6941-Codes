...
# # This version is for the scenario that the robot is on the right and does switch during auto
class MyRobot(wpilib.IterativeRobot):
    ...
    def autonomousPeriodic(self):
        if self.gameData[0:1] == "L":
            ...
            while timer.get() < 1.2:
                ...
                self.ilow_motor.set(-0.95) # intake      
                self.aSolenoidLow.set(1)
                self.myRobot.tankDrive(0.7,0.7)     # To make sure it's not accelerating to fast
            while timer.get() < 3:
                self.myRobot.tankDrive(0,0)         # This is problematic. Should have used "pass". Otherwise the motor stops immediately and the robot won't reach the distance.
            timer.reset()
            timer.start()
            while timer.get() < 0.85:           # Turn 90 degrees. Left = 0.82  was the original value we got during tests.
                self.ihigh_motor.set(0.6) # intake
                self.ilow_motor.set(-0.95) # intake
                self.myRobot.tankDrive(0.7,-0.7) # Turing left, even though it looks like turning right
            while timer.get() < 1.4:
    ...
    def teleopPeriodic(self):
        if self.isOperatorControl() and self.isEnabled():
            ...
            r = (steering**3)*mod               # To make it harder to turn when speed is large in order to assure a smooth drive and prevent drifting
